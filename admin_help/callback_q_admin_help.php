...
$file = __DIR__ . "/../json_data/admin_help/editMsgID.json";
...
$get_help_files = file_get_contents(__DIR__ . '/../json_data/admin_help/slangword.json');
$helper = json_decode($get_help_files, true);


$reply = $helper[$adanParse[1]];
$option = array(
    array($telegram->buildInlineKeyBoardButton("kembali", $url = "", $callback_data = '/help_admin_i@fadhil_riyanto_bot'))
);
...
{
    $file = __DIR__ . "/../json_data/admin_help/editMsgIDdelete.json";
    $anggota = file_get_contents($file);
    $data = json_decode($anggota, true);
    foreach ($data as $d) {
        if ($d['userid'] == $userID) {
