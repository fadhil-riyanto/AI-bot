...
if ($azanHilangcommand == null) {
    $reply = "maaf, anda harus memasukkan input header yang ingin dicari.";
    $content = array('chat_id' => $chat_id, 'text' => $reply, 'reply_to_message_id' => $message_id, 'parse_mode' => 'html', 'disable_web_page_preview' => true);
    $telegram->sendMessage($content);
} else {

    $getfilenya = json_decode(file_get_contents(__DIR__ . '/../json_data/serv/header.json'));
    foreach ($getfilenya as $kode_http) {
        if (strtolower($kode_http->header) == strtolower($udahDiparse)) {
            $reply = 'Deskripsi : ' . $kode_http->description . PHP_EOL . PHP_EOL .
